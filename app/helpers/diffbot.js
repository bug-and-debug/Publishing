...
Diffbot.prototype.analyze = function (options, callback) {
  ...
  var diffbot_url = "http://api.diffbot.com/v3/analyze?token=" + this.token + "&url=" + encodeURIComponent(options.uri);
...
Diffbot.prototype.frontpage = function (options, callback) {
  ...
  var diffbot_url = "http://api.diffbot.com/v3/frontpage?token=" + this.token + "&url=" + encodeURIComponent(options.uri) + "&formd=json";
...
const diffbot = new Diffbot(config.DiffBotAPI.token);
exports.analyze = async function(url) {
	return new Promise( (resolve, reject) => {
		diffbot.analyze({uri: url}, (err, response) => {
			if(err) reject(err);
