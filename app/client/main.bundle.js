...
AppConfig.API_ENDPOINT = "http://18.219.41.26:4000/api/v1";
...
var D3Service = (function () {
    ...
    D3Service.prototype.init = function (svgSelector, zoomEnabled, centralOffset) {
        ...
        if (zoomEnabled) {
            ...
            __WEBPACK_IMPORTED_MODULE_0_d3_index__["f" /* select */]("body").on("keydown", function () {
                if (__WEBPACK_IMPORTED_MODULE_0_d3_index__["b" /* event */].keyCode == 38) {
                    _this.viewNextArticle(false);
                }
                else if (__WEBPACK_IMPORTED_MODULE_0_d3_index__["b" /* event */].keyCode == 40) {
    ...
    D3Service.prototype.reskinNodes = function (nodes) {
        ...
        this.svgElements.imgsArticle
            .attr('width', function (d) { return d.style.height - 35; })
            .attr('height', function (d) { return d.style.height - 35; });
    ...
    D3Service.prototype.visualize = function () {
        ...
        this.svgElements.tspansArticleTitle1 = this.svgElements.textsArticleTitle
            ...
            .attr('x', 0)
            .attr('dy', 15)
            .text(function (d) { return d.title.substring(0, 35); })
            .attr('data-index', function (d) { return d.nodeData.nodeIndex; });
        this.svgElements.tspansArticleTitle2 = this.svgElements.textsArticleTitle
            .append('tspan')
            .attr('x', 0)
            .attr('dy', 30)
            .text(function (d) { return __WEBPACK_IMPORTED_MODULE_1_lodash__["truncate"](d.title.substring(35), { 'length': 35 }); })
            .attr('data-index', function (d) { return d.nodeData.nodeIndex; });
        ...
        this.svgElements.tspansArticleBody1 = this.svgElements.textsArticleBody
            ...
            .attr('x', 0)
            .attr('dy', 15)
            .text(function (d) { return d.body.substring(0, 42); })
            .attr('data-index', function (d) { return d.nodeData.nodeIndex; });
        this.svgElements.tspansArticleBody2 = this.svgElements.textsArticleBody
            .append('tspan')
            .attr('x', 0)
            .attr('dy', 30)
            .text(function (d) { return __WEBPACK_IMPORTED_MODULE_1_lodash__["truncate"](d.body.substring(42), { 'length': 42 }); })
            .attr('data-index', function (d) { return d.nodeData.nodeIndex; });
    ...
    D3Service.prototype.viewNextArticle = function (direction /* boolean: true - next, false - prev) */) {
        ...
        if (this.currentArticleIndex < 0)
            this.currentArticleIndex = this.currentArticleIndex = articleIndices.length - 1;
    ...
    D3Service.prototype.ticked = function () {
        ...
        this.svgElements.tspansArticleTitle1
            .attr('x', function (d) { return d.x + 125; })
            .attr('y', function (d) { return d.y; });
        ...
        this.svgElements.linesArticle
            ...
            .attr('y1', function (d) { return d.y + d.style.height - 33; })
            .attr('x2', function (d) { return d.x + d.style.width; })
            .attr('y2', function (d) { return d.y + d.style.height - 33; });
        this.svgElements.imgsArticleSite
            .attr('x', function (d) { return d.x + 5; })
            .attr('y', function (d) { return d.y + d.style.height - 30; });
        this.svgElements.textsArticleSite
            .attr('x', function (d) { return d.x + 35; })
            .attr('y', function (d) { return d.y + d.style.height - 16; });
        this.svgElements.textsArticleTime
            .attr('x', function (d) { return d.x + d.style.width - 145; })
            .attr('y', function (d) { return d.y + d.style.height - 16; });
        this.svgElements.editLinksArticle
            .attr('x', function (d) { return d.x + d.style.width - 30; })
            .attr('y', function (d) { return d.y + d.style.height - 16; });
        this.svgElements.textsArticleMenu
            .attr('x', function (d) { return d.x + d.style.width - 5; })
    ...
    D3Service.prototype.bound = function (alpha /*ignored*/) {
        var articleIndex = this.nextArticle(-1);
        while (articleIndex < this.nodes.length) {
            var parent = this.nodes[articleIndex];
            var i = articleIndex + 1;
            for (; i < this.nodes.length && this.nodes[i].shape != 'article'; i++) {
                var oneNode = this.nodes[i];
                ...
                if (parent.nodeData.clustersPopulation) {
                    var regionPolygon = __WEBPACK_IMPORTED_MODULE_3__shared_services_dataService__["a" /* DataService */].polygon(parent, oneNode.nodeData.clusterName);
                    var parentRectangle = __WEBPACK_IMPORTED_MODULE_5_kld_intersections__["Shapes"].rectangle(parent.x, parent.y, parent.style.width, parent.style.height);
                    if (__WEBPACK_IMPORTED_MODULE_3__shared_services_dataService__["a" /* DataService */].checkPointInRectangle(new __WEBPACK_IMPORTED_MODULE_5_kld_intersections__["Point2D"](oneNode.x, oneNode.y), parentRectangle)) {
                        return;
                    }
                    var oneNodeShape = __WEBPACK_IMPORTED_MODULE_5_kld_intersections__["Shapes"].rectangle(oneNode.x, oneNode.y, oneNode.style.width, oneNode.style.height);
                    var intersection = __WEBPACK_IMPORTED_MODULE_5_kld_intersections__["Intersection"].intersect(oneNodeShape, regionPolygon);
                    var move = void 0;
                    if (intersection.status == 'Intersection') {
                        // is intersecting with border line, put it inside the region with least amount of move
                        move = __WEBPACK_IMPORTED_MODULE_3__shared_services_dataService__["a" /* DataService */].putInside(oneNode, regionPolygon);
                        oneNode.x += move.x;
                        oneNode.y += move.y;
                    }
                    else {
                        var centerPoint = new __WEBPACK_IMPORTED_MODULE_5_kld_intersections__["Point2D"](oneNode.x + oneNode.style.width / 2, oneNode.y + oneNode.style.height / 2);
                        if (!__WEBPACK_IMPORTED_MODULE_3__shared_services_dataService__["a" /* DataService */].checkPointInPolygon(centerPoint, regionPolygon)) {
                            // is outside, put it inside the region
                            // console.log('Forcefull put inside')
                            var enterPosition = __WEBPACK_IMPORTED_MODULE_3__shared_services_dataService__["a" /* DataService */].enterPosition(parent, oneNode.nodeData.clusterName, i);
    ...
    D3Service.prototype.bubbleSwitch = function (alpha /*ignored*/) {
        var articleIndex = this.nextArticle(-1);
        while (articleIndex < this.nodes.length) {
            var nextArticleIndex = this.nextArticle(articleIndex);
            for (var i = 1; i < nextArticleIndex - articleIndex - 1; i++) {
                for (var j = i + 1; j < nextArticleIndex - articleIndex; j++) {
                    var oneNode = this.nodes[articleIndex + i];
                    var otherNode = this.nodes[articleIndex + j];
                    if (otherNode.x == null || otherNode.y == null || oneNode.x == null || oneNode.y == null) {
                        console.log('This should not happen');
                        continue;
                    }
                    var clusters = void 0;
                    var nodeOne = void 0, nodeTwo = void 0;
                    ...
                    var parentNode = this.nodes[articleIndex];
                    if ((clusters[0] == 'g11' && clusters[1] == 'g6') || (clusters[0] == 'g3' && clusters[1] == 'g7')) {
                        var pullPoint = __WEBPACK_IMPORTED_MODULE_3__shared_services_dataService__["a" /* DataService */].pullPoint(parentNode, clusters[0], 0);
                        var distanceOne = __WEBPACK_IMPORTED_MODULE_3__shared_services_dataService__["a" /* DataService */].distance(pullPoint, { x: nodeOne.x + nodeOne.style.width / 2, y: nodeOne.y + nodeOne.style.height / 2 });
                        var distanceTwo = __WEBPACK_IMPORTED_MODULE_3__shared_services_dataService__["a" /* DataService */].distance(pullPoint, { x: nodeTwo.x + nodeTwo.style.width / 2, y: nodeTwo.y + nodeTwo.style.height / 2 });
                        if (distanceOne > distanceTwo + 10) {
                            // switch position
                            var oneCenter = { x: nodeOne.x + nodeOne.style.width / 2, y: nodeOne.y + nodeOne.style.height / 2 };
                            var twoCenter = { x: nodeTwo.x + nodeTwo.style.width / 2, y: nodeTwo.y + nodeTwo.style.height / 2 };
    ...
    AuthService.prototype.login = function (provider) {
        switch (true) {
            ...
            case provider == 'FACEBOOK':
                this._auth.signIn(__WEBPACK_IMPORTED_MODULE_1_angular4_social_login__["FacebookLoginProvider"].PROVIDER_ID);
                break;
